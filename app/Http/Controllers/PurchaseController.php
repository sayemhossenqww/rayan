...
class PurchaseController extends Controller
{
    public function index(Request $request)
    {
        ...
        for($i = 0; $i < count($purchases); $i++) {
            $purchases[$i]->average_cost = PurchaseDetail::where('purchase_id', $purchases[$i]->id)->avg('cost') ?? 0;
            $purchases[$i]->last_cost = PurchaseDetail::where('purchase_id', $purchases[$i]->id)->orderBy('id', 'desc')->first()->cost ?? 0;
        }
    ...
    public function store(Request $request)
    {
        ...
        if (!empty($supplierName)) {
            ...
            if (!$supplier) {
                return back()->with('error', __('Supplier not found'));
            }
        }
