...
class OrderController extends Controller
{
    ...
    public function update(Request $request, Order $order)
    {
        ...
        foreach ($cart as $cartItem) {
            ...
            if ($product) {
                $size = ($cartItem->price_type === 1) ? $product->count_per_box : 1;
                ...
                $oldPriceType = $cartItem->old_price_type;
                $old_size = ($oldPriceType === 1) ? $product->count_per_box : 1;
                ...
                $order_detail->box_price = $cartItem->box_price;
                $order_detail->unit_price = $cartItem->unit_price;
                $order_detail->price_type = $cartItem->price_type;
                $order_detail->cost = $product->cost;
                $order_detail->created_at = $order->created_at;

                $order_detail->total = $quantity * $productPrice;
                $order_detail->total_cost = $quantity * $product->cost * $size;


                if ($product->track_stock) {
                    $currentInStock =  $product->in_stock;
                    $new_stock = $oldQuantity * $old_size;
                    $product->in_stock = $currentInStock + $new_stock - $quantity * $size;
                    $product->save();
                }
    ...
    public function store(Request $request)
    {
        ...
        foreach ($cart as $cartItem) {
            ...
            if ($product) {
                ...
                $order_detail->total = $quantity * $productPrice;
                $order_detail->total_cost = $quantity * $productCost * $size;
                $order_detail->product()->associate($product);
                $order_detail->order()->associate($order);
                $order_detail->save();
                ...
                if ($product->track_stock) {
                    $product->in_stock -= ($quantity * $size);
                    $product->save();
                }
        ...
        $barcode = new DNS1D(); // Instantiate the class
        $barcodeSVG = $barcode->getBarcodeSVG($order->number, 'C128', 2, 30, 'black', false); // Call the method on the instance
