...
class PointOfSaleEdit extends Component<Props, State> {
    ...
    setupForEditing = async (): Promise<void> => {
        ...
        this.setState({ isPaid: order.paid == 1 ? true : false });
        ...
        order.order_details.map((item: any) => {
            let cartItem: ICartItem = {
                ...
                old_price_type: item.price_type,
                image_url: item.product.image_url,
                // barcode: item.barcode,
                box_barcode: item.box_barcode,
                unit_barcode: item.unit_barcode,
                superdealer_barcode: item.superdealer_barcode,
                wholesale_barcode: item.wholesale_barcode,
                pricepergram_barcode: item.pricepergram_barcode,
                priceperkilogram_barcode: item.priceperkilogram_barcode,
                // sku: item.sku,
                box_sku: item.box_sku,
                unit_sku: item.unit_sku,
                superdealer_sku: item.superdealer_sku,
                wholesale_sku: item.wholesale_sku,
                pricepergram_sku: item.unit_sku,
                priceperkilogram_sku: item.unit_sku,
    ...
    addToCart = (product: IProduct, price_type: number = this.state.currentPrice): void => {
        ...
        let cartItem: ICartItem = {
            ...
            quantity: 1,
            old_quantity: 0,
            old_price_type: this.state.currentPrice,
            ...
            <div className="modal zoom-out-entrance" id="myModal" tabIndex={-1} aria-labelledby="myModalLabel" aria-hidden="true">
                <div className="modal-dialog modal-dialog-centered">
                    <div className="modal-content">
                        ...
                        {/* <div className="modal-footer">
                            <div className="d-flex justify-content-between w-100 user-select-none cursor-pointer" onClick={() => this.handleSelectRadio(3)}>
                                <div>
                                    {priceFilterList[3]}
                                </div>
                                <input type="radio" checked={this.state.currentPrice === 3}/>
                            </div>
                        </div>
                        <div className="modal-footer">
                            <div className="d-flex justify-content-between w-100 user-select-none cursor-pointer" onClick={() => this.handleSelectRadio(4)}>
                                <div>
                                    {priceFilterList[4]}
                                </div>
                                <input type="radio" checked={this.state.currentPrice === 4}/>
                            </div>
                        </div> */}
                    </div>
                </div>
