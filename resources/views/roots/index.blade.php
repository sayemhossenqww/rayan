@extends('layouts.app')
@section('title', __('Roots'))

@section('content')
    <div class="d-flex align-items-center justify-content-center mb-3">
        <div class="flex-grow-1">
            <x-page-title>@lang('Roots')</x-page-title>
        </div>
        <a href="{{ route('roots.create') }}"
    ...
    <x-card>
        <x-table id="categories-table">
            <x-thead>
                <tr>
                    <x-th>@lang('Root Name')</x-th>
                    <x-th>@lang('Area Name')</x-th>
                    <x-th>@lang('City Name')</x-th>
                    <x-th></x-th>
                </tr>
            </x-thead>
        </x-table>
                ...
                columns: [{
                    ...
                    },
                    {
                        ...
                        data: function(data, type, dataToSet) {
                            var editUrl = "{{ route('roots.edit', ':id') }}";
                            var deleteUrl = "{{ route('roots.destroy', ':id') }}";
